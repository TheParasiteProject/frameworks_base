    ...
    private boolean applyAspectRatio(Rect outBounds, Rect containingAppBounds,
        ...
        if (containingRatio - aspectRatioToApply > ASPECT_RATIO_ROUNDING_TOLERANCE) {
            if (mAtmService.shouldForceLongScreen(packageName)
                || mAtmService.shouldForceCutoutFullscreen(packageName)) {
                // Use containingAppWidth/Height for maxActivityWidth/Height
                // when force long screen and cutout force full screen
